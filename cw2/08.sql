...
SELECT COALESCE(LJ.ocust, RJ.custid) as CustomerID, COALESCE(LJ.OrderCost, 0.00)
FROM (	SELECT O.ocust, CAST(SUM(COALESCE(PD.OrderCost,0)) AS Numeric(8,2)) AS OrderCost
		FROM Orders O 	
